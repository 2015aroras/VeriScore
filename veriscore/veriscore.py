...
class VeriScorer(object):
    def __init__(
        ...
        self.use_external_extraction_model = use_external_extraction_model
        self.use_external_verification_model = use_external_verification_model
        ...
        self.demon_dir = os.path.join(self.data_dir, "demos")
        self.model_name_verification = model_name_verification
    ...
    def get_veriscore(self, data, input_file_name):
        ...
        claim_extractor = ClaimExtractor(
            self.model_name_extraction,
            cache_dir=self.cache_dir,
            use_external_model=self.use_external_extraction_model,
        )
            ...
            for dict_item in tqdm(data):
                ...
                if "question" in dict_item and dict_item["question"]:
                    ...
                    (
                        ...
                        prompt_tok_cnt,
                        response_tok_cnt,
                    ) = claim_extractor.qa_scanner_extractor(question, response)
                else:
                    question = ""
                    (
                        snippet_lst,
        ...
        claim_verifier = ClaimVerifier(
            model_name=self.model_name_verification,
            label_n=self.label_n,
            cache_dir=self.cache_dir,
            demon_dir=self.demon_dir,
            use_external_model=self.use_external_verification_model,
        )
        ...
        print(
            f"Total cost: {total_prompt_tok_cnt * 10 / 1e6 + total_resp_tok_cnt * 30 / 1e6}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name_extraction", type=str, default="gpt-4-0125-preview"
    )
